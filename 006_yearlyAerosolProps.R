...
if (!file.exists("../results/values/yearly_aerosolProps.rds")){
  ...
  saveRDS(df, file = "../results/values/yearly_aerosolProps.rds")
} else {
  df = readRDS("../results/values/yearly_aerosolProps.rds")
}
...
meanplot = ggplot(df, aes(x=year))+
  ...
  #           mapping= aes(x=2017, y=0, label= paste("slope: ", round(slope, 2), sep="")))+
  # geom_text(data=regCoef,
  #           mapping= aes(x=2014, y=0, label= paste("R²: ", round(r, 4), sep="")))+
  theme_minimal()+
