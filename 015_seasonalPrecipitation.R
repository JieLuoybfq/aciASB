...
seasonalMeans = lapply(paraStacks, function(x){
  ...
  tmpLS = lapply(years, function(y){
    ...
    tmpRas = x[[which(l %in% layer_names)]]
    # get january layer for next year
    jan = x[[grep(paste0("chirps_Jan_", as.character(nextY)), l)]]
    feb = x[[grep(paste0("chirps_Feb_", as.character(nextY)), l)]]
    ...
    s1 = clusterR(tmpRas[[which( layer_months %in% seasons$s1 == TRUE)]], calc, args = list(fun=sum, na.rm = TRUE))
    s2 = clusterR(tmpRas[[which( layer_months %in% seasons$s2 == TRUE)]], calc, args = list(fun=sum,na.rm = TRUE))
    s3 = clusterR(tmpRas[[which( layer_months %in% seasons$s3 == TRUE)]], calc, args = list(fun=sum,na.rm = TRUE))
    s4 = clusterR(tmpRas[[which( layer_months %in% seasons$s4 == TRUE)]], calc, args = list(fun=sum,na.rm = TRUE))
    endCluster()
