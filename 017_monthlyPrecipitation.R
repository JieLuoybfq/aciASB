...
if (!file.exists("../results/values/monthly_Precipitation.rds")){
  ...
  monthlySums = lapply(paraStacks, function(x){
    ...
    xsum = do.call(rbind, tmpLS)
    return(xsum)
  })
  
  # preparation for plotting
  names(monthlySums) = paras
  for (i in 1:length(paras)){
    monthlySums[[paras[i]]]$type = paras[i]
  }
  
  df = as_tibble(do.call(rbind, monthlySums))
  saveRDS(df, file = "../results/values/monthly_Precipitation.rds")
} else {
  df = readRDS("../results/values/monthly_Precipitation.rds")
}
...
# saving plot to disk
ggsave(plot=meanplot, file = "../results/plots/precipitation_monthly.png", dpi=600, device="png", width = 20, height = 10)
