...
getParCor = function(x){
  if (sum(is.na(x))== 0){ #
    ...
    result = pcor.test(x=parax, y=paray, z=parac, method = "spearman")
    return(c(result$estimate, result$p.value))
  }else{
    return(c(NA,NA))
  }
...
getParCorMulti = function(x){
  if (sum(is.na(x))== 0){ #
    ...
    result = pcor.test(x = tmp[ ,1], y = tmp[ ,2], z = tmp[ ,-c(1,2)], method = "spearman")
    return(c(result$estimate, result$p.value))
  }else{
    return(c(NA,NA))
  }
...
na_mask = stack(valsx, valsy, valsc)
beginCluster(6)
na_mask = clusterR(na_mask, calc, args=list(fun=sum, na.rm=F))
endCluster()

valsx[is.na(na_mask)] = NA
valsy[is.na(na_mask)] = NA
valsc[is.na(na_mask)] = NA
